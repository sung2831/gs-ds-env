...
def get_dataset_item(table_name, project_hashkey, file_hashkey):
    try:
        ...
        # 키 조건 설정
        key = {"project_hashkey": project_hashkey, "file_hashkey": file_hashkey}
        # 항목 조회
        response = table.get_item(Key=key)
...
def get_model_repo_item(table_name, model_hashkey):
    try:
        ...
        # 키 조건 설정
        key = {"model_hashkey": model_hashkey}
        # 항목 조회
        response = table.get_item(Key=key)
...
def download_s3_files_to_directory(
    bucket_name: str, s3_prefix: str, local_dir: str = "."
):
    """
...
def upload_directory_to_s3(local_directory, bucket, s3_prefix):
    ...
    for root, _, files in os.walk(local_directory):
        ...
        for file in files:
            ...
            s3_file_path = os.path.join(s3_prefix, relative_file_path).replace(
                "\\", "/"
            )  # S3 경로에 슬래시 사용
            ...
            try:
                ...
                print(
                    f"Failed to upload {local_file_path} to s3://{bucket}/{s3_file_path}: {e}"
                )
